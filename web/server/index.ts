...
import { authMiddleware, createAuthRoutes, isAuthenticated } from "./auth.js";
...
app.get("/manifest.json", (c) => {
  const manifest = {
    name: "Neuron Spark Code",
    short_name: "Neuron Spark Code",
...
// Force-disable legacy PWA behavior. Existing clients may still have an old
// registration; serving a self-destructing worker makes the browser unregister it.
app.get("/sw.js", (c) => {
...
const server = Bun.serve<SocketData>({
  ...
  async fetch(req, server) {
    ...
    const hasCustomAuthSession = isAuthenticated(req.headers.get("cookie"));
